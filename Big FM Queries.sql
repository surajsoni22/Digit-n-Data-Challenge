...
Select t1.HostID, t1.showname, t2.hostname from Shows t1 join Hosts t2 on t1.HostID = t2.HostID where t2.HostName = 'Vrajesh Hirjee';

Select t1.showid, t2.ShowName, COUNT(*) AS TotalAward from Awards t1 inner join Shows t2 on t1.ShowID = t2.ShowID  Group by t2.ShowID;

select t2.showid, t2.showname, t1.partnername from partnerships t1 inner join shows t2 on t1.PartnershipID = t2.ShowID where PartnerName = 'spotify';
...
select hostname, showcount as total_count from hosts;
...
select hostname, showcount, dense_rank() over(order by showcount desc) as Host_Rank from hosts;